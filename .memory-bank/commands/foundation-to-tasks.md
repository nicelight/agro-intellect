...
- `.memory-bank/contracts/foundation-critical-path.md` when present or linked from
  `.memory-bank/foundation.md`
...
- Foundation Gate Task: pending_/foundation-to-tasks|TASK-<NNN>-FT-000-W-<N>|not_required
```
...
- `Foundation Required: true` from `/spec-design` may start with
  `Foundation Gate Task: pending_/foundation-to-tasks`; this command must decide
  the task queue, create at least one implementation/probe task and exactly one
  final foundation gate task, then replace the pending marker with the concrete
  gate task id before handoff.
...
If `.memory-bank/contracts/foundation-critical-path.md` is present, every
foundation task that touches the critical path must cite the relevant `C-FND-*`
contract records in `normative_inputs`, `constraints`, or
`verification_targets` as appropriate. The final foundation gate task must verify
the full contract set required by `.memory-bank/foundation.md`.
...
- ensure `.memory-bank/foundation.md` Gate Anchors name the final gate task
  selected by this command, or `not_required`
