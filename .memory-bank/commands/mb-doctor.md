...
- `pending_/foundation-to-tasks` is allowed immediately after `/spec-design`
  before a foundation task queue exists; it is invalid after `/foundation-to-tasks`
  creates tasks or in strict task-selection mode.
- When foundation is required and a concrete gate is named, the final foundation
  gate task exists, is indexed, has `feature: "FT-000"`, and every non-`FT-000`
  product task depends on it directly or transitively.
...
- `FOUNDATION_GATE_TASK_PENDING` in `--strict` or after task queue creation
