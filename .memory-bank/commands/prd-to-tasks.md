...
If foundation is required but `Foundation Gate Task` is still
`pending_/foundation-to-tasks`, or the final gate task is missing, not indexed,
not `done`, or not an `FT-000` task, stop and route to `/foundation-to-tasks`,
