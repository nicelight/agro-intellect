...
- `.memory-bank/contracts/foundation-critical-path.md` when a required Foundation Dev Path needs executable contract records before `/foundation-to-tasks`
...
- when `/spec-design` sets `Foundation Required: true`, set
  `Foundation Gate Task: pending_/foundation-to-tasks`; do not guess task count
  or the final gate task id
- when the foundation path crosses shared event, agent, safety, state, audit, or
  export boundaries, create or update a compact
  `.memory-bank/contracts/foundation-critical-path.md` contract for
  `/foundation-to-tasks` instead of creating task records or implementation
  plans
...
- foundation next step: `/foundation-to-tasks`, or `none` when
  `Foundation Gate Task: not_required`; `/foundation-to-tasks` owns replacing
  `pending_/foundation-to-tasks` with a concrete final gate task id
