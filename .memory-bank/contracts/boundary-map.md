...
| Safe request / Safety Gate -> Safety & Task Loop | Create and transition check/measurement tasks, pending approvals, human-performed `action_task` records, and follow-up outcomes. | Approved or low-risk backend request into task/approval/follow-up authority. | /prd then /spec-design | Tasks, approvals, and outcomes retain Plant/evidence refs; archived Plants make them non-operative; restore never auto-resumes work. No automated actuation. | Exact task, approval, outcome, and follow-up contracts belong to `/prd-to-tasks FT-012`. |
