...
Run global `/spec-design` before this feature is task-decomposed. Then run `/prd-to-tasks FT-006`; it must define exact runtime state ownership, timeline event taxonomy, history projections, export refs, and retention behavior during its feature-level SDD design phase before writing tasks. Use standalone `/spec-improve FT-006` only for repair or advanced refresh without task generation.
