...
- `TASK-034-T3-FT-009-W1`: `done`.
- `TASK-035-T3-FT-009-W2`: `done`.
- Feature `lifecycle` remains `planned`.
- The persistence boundary refreshes locked authoritative photo ownership
  before provenance comparison, rejects stale or cross-Plant binding, and
  preserves strict cursor errors without widening the public contract.
...
- Blockers: none. Provider/model/base URL, credentials, egress, network, and
  live smoke are not current deterministic acceptance inputs.
