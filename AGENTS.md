...
## Communication
- Always answer the operator in Russian.
- Keep stable English technical terms as-is when they are established project/tooling terms, for example `Memory Bank`, `task`, `feature`, `workflow`, `quality gates`, `backend`, `frontend`, `runtime`, `Agent Chat Bus`, and `MessageEnvelope`.
