...
exports.DIGIT_INDEX_ACTION_IDS = exports.KEYBINDING_DEFINITIONS = void 0;
exports.agentTabActionId = agentTabActionId;
exports.isDigitIndexActionId = isDigitIndexActionId;
exports.getKeybindingPlatform = getKeybindingPlatform;
exports.isKeybindingActionId = isKeybindingActionId;
exports.normalizeKeybinding = normalizeKeybinding;
exports.isDoubleTapBinding = isDoubleTapBinding;
exports.normalizeKeybindingList = normalizeKeybindingList;
exports.normalizeKeybindingListForAction = normalizeKeybindingListForAction;
exports.normalizeKeybindingArrayForAction = normalizeKeybindingArrayForAction;
exports.keybindingFromInput = keybindingFromInput;
exports.keybindingFromInputForAction = keybindingFromInputForAction;
exports.getEffectiveKeybindingsForAction = getEffectiveKeybindingsForAction;
exports.getKeybindingDefinition = getKeybindingDefinition;
exports.normalizeTerminalShortcutPolicy = normalizeTerminalShortcutPolicy;
exports.isKeybindingAllowedInTerminal = isKeybindingAllowedInTerminal;
exports.isKeybindingPotentialTerminalConflict = isKeybindingPotentialTerminalConflict;
exports.keybindingIsActiveInContext = keybindingIsActiveInContext;
exports.keybindingMatchesInput = keybindingMatchesInput;
exports.keybindingMatchesAction = keybindingMatchesAction;
exports.matchKeybindingDigitIndex = matchKeybindingDigitIndex;
exports.formatKeybinding = formatKeybinding;
exports.formatKeybindingList = formatKeybindingList;
exports.findKeybindingConflicts = findKeybindingConflicts;
const tui_agent_display_names_1 = require("./tui-agent-display-names");
exports.KEYBINDING_DEFINITIONS = [
    {
...
function buildAgentTabKeybindingDefinitions() {
    return tui_agent_display_names_1.ALL_TUI_AGENTS.map((agent) => ({
        id: agentTabActionId(agent),
        title: `New ${tui_agent_display_names_1.TUI_AGENT_DISPLAY_NAMES[agent]} tab`,
        group: 'Agents',
        scope: 'tabs',
        searchKeywords: [
            'shortcut',
            ...
            'launch',
            agent,
            tui_agent_display_names_1.TUI_AGENT_DISPLAY_NAMES[agent].toLowerCase()
        ],
        defaultBindings: platformBindings([])
    }));
}
const DEFINITIONS_BY_ID = new Map(exports.KEYBINDING_DEFINITIONS.map((definition) => [definition.id, definition]));
const DEFINITION_IDS = new Set(exports.KEYBINDING_DEFINITIONS.map((definition) => definition.id));
...
exports.DIGIT_INDEX_ACTION_IDS = [
    'tab.selectByIndex',
    'workspace.selectByIndex'
];
const DIGIT_INDEX_ACTION_ID_SET = new Set(exports.DIGIT_INDEX_ACTION_IDS);
...
function isKeybindingActionId(value) {
    return DEFINITION_IDS.has(value);
}
...
function normalizeKeybindingList(input) {
...
function shouldUseMacOptionComposedCaptureFallback(input, platform) {
    ...
    return ((physicalToken.length === 1 && physicalToken >= 'A' && physicalToken <= 'Z') ||
...
function keybindingFromInputWithOptions(input, platform, options = {}) {
    if (input.doubleTapModifier) {
        return normalizeKeybindingWithOptions(`DoubleTap+${canonicalDoubleTapToken(input.doubleTapModifier, platform)}`, options);
    }
...
function keybindingFromInput(input, platform) {
...
function modifierStateMatches(parsed, input, platform) {
    ...
    return (hasModifier(input, 'meta') === expected.meta &&
...
function shouldUseMacOptionLetterPhysicalFallback(parsed, input, platform) {
    ...
    return (getKeybindingPlatform(platform) === 'darwin' &&
        parsed.alt &&
        hasModifier(input, 'alt') &&
...
function keyMatches(parsedKey, input, parsed, platform) {
    ...
    if (parsedKey === 'NumpadAdd' || parsedKey === 'NumpadSubtract') {
        return (numpadCodeKeyTokenFromInput(input) === parsedKey ||
...
function keybindingMatchesInput(binding, input, platform) {
    ...
    if (parsed.doubleTapModifier) {
        return (input.doubleTapModifier !== undefined &&
...
function keybindingConflictIdentity(binding, platform) {
    const parsed = parseKeybinding(binding);
    if (!parsed?.doubleTapModifier) {
        return binding;
    }
...
function findKeybindingConflicts(platform, overrides, options = {}) {
    ...
    for (const definition of exports.KEYBINDING_DEFINITIONS) {
